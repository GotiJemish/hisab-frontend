...
//   const isRegister = searchParams.get("userType") === "newUser";
//   // Refs for 6 inputs
...
//       const email =  localStorage.getItem(isRegister?"registerEmail":"forgetPasswordEmail");
//       const payload = { email, otp };

//       const response = await apiClient.post(`auth/${isRegister?"verify-otp":"verify-forgot-otp"}/`, payload);
...
//         router.push(isRegister?"/":"/new-password");
//       } else {
...
//       setLoading(true);
//       const email = localStorage.getItem(isRegister?"registerEmail":"forgetPasswordEmail");
//        const first_name= localStorage.setItem("f_name", form.fname);
//        const last_name=localStorage.setItem("l_name", form.lname);
...
//       const res = await apiClient.post(`auth/${isRegister?"register":"forgot-password"}/`, isRegister?{email,first_name,last_name}:{ email });
//       if (res?.status === 200) {
...
// app/(auth)/otp-verify/page.jsx (or wherever you keep it)
"use client";
import { ChevronLeftIcon } from "@/icons";
import Link from "next/link";
import React, { useEffect, useRef, useState } from "react";
...
const OtpVerify = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const email = localStorage.getItem(isRegister ? "registerEmail" : "forgetPasswordEmail");
      if (!email) {
        showToast({ message: "Email not found. Please start again.", type: "error" });
        return;
      }
      ...
      const response = await apiClient.post(
        `auth/${isRegister ? "verify-otp/register" : "verify-forgot-otp"}/`,
        payload
      );

      if (response?.status === 200 || response?.status === 201) {
        showToast({ message: "OTP verified. Proceed to set your password.", type: "success" });

        // mark verified and navigate to new-password page with context
        localStorage.setItem("otpVerified", "true");
        const nextRoute = `/new-password?userType=${isRegister ? "newUser" : "forgot"}`;
  ...
  const handleResend = async (e) => {
    ...
    try {
      ...
      // For registration we include first & last name as that endpoint expects it in your earlier code.
      const body = isRegister ? { email, first_name, last_name } : { email };
      const res = await apiClient.post(`auth/${isRegister ? "register" : "forgot-password"}/`, body);
